...
#	endif
		...
		message_warning(_("%s: File seems to have been moved, "
				"not removing"), tuklib_mask_cntrl(name));
	else
#endif
...
io_copy_attrs(const file_pair *pair)
{
	...
	if (fchown(pair->dest_fd, pair->src_st.st_uid, (gid_t)(-1))
		...
		message_warning(_("%s: Cannot set the file owner: %s"),
				tuklib_mask_cntrl(pair->dest_name),
				strerror(errno));
...
io_open_src_real(file_pair *pair)
{
	...
	if (!follow_symlinks) {
		...
		if (lstat(pair->src_name, &st)) {
			message_error(_("%s: %s"),
					tuklib_mask_cntrl(pair->src_name),
					strerror(errno));
			return true;
...
error_msg:
	message_error(_("%s: %s"), tuklib_mask_cntrl(pair->src_name),
			strerror(errno));
error:
